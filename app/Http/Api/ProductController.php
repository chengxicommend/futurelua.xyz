...
class ProductController extends Container
{
    ...
    public function getProducts($request, $response)
    {
        ...
        if ($verify) {
            ...
            if ($user->banned == 1) {
                $ret = json_encode(['success' => false, 'error' => "banned"]);
            }else{
                    ...
                    if (Utils::isGroupMember($user, $product->group_id) || Utils::isGroupAdmin($user, $product->group_id) || Permissions::isAdmin($user)) {
                        $ret = json_encode(
                            ['success' => true,
                                ...
                                'code' => Cipher::encrypt(ProductController::getProductCode($user, $product),Cipher::$STATIC_KEY1, Cipher::$STATIC_KEY2)
                            ]);
                    } else {
...
EOT;
        ...
        return [
            ...
            'code' => Cipher::encrypt($content,Cipher::$STATIC_KEY1, Cipher::$STATIC_KEY2),
            'version' => $product->version
        ];
    }
}